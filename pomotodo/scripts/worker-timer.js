...
onmessage = function (e) {
    ...
    function workerTick() {
        const CURRENT_TIME = new Date();
        const TIME_REMAINING_MS = END_TIME.getTime() - CURRENT_TIME.getTime();

        // Timer finished
        if (TIME_REMAINING_MS <= 0) {
            postMessage(["00", "00", END_TIME.getTime() - START_TIME.getTime(), 0]);
            return;
        }
        ...
        const MILLISECONDS_PASSED = CURRENT_TIME.getTime() - START_TIME.getTime();
        const MILLISECONDS_TO_NEXT_SECOND = 1000 - (MILLISECONDS_PASSED % 1000);

        postMessage([MINUTES_REMAINING, secondsRemaining, MILLISECONDS_PASSED, MILLISECONDS_TO_NEXT_SECOND]);

        setTimeout(workerTick, MILLISECONDS_TO_NEXT_SECOND);
    }

    workerTick(); // Initial call
};
